...
- **Importar com IA (botão "Importar com IA"):** envie uma planilha **.xlsx** onde cada **aba é o carnê de pagamento de um cliente** (nome da aba = nome do cliente). A IA localiza o nome do imóvel/empreendimento escrito dentro da planilha (ex: título "RESIDENCIAL VIVERDE") e identifica a tabela de parcelas de cada aba (número, data de vencimento, valor, se já foi paga) mesmo que a estrutura varie de aba para aba — a extração das linhas em si é determinística (não alucina dados). Mostra uma prévia com cliente, imóvel, quantidade de parcelas, quantas já pagas e valor total antes de confirmar. Ao confirmar, cria o cliente **e já gera as parcelas em Contas a Receber** (Financeiro), com status pago/pendente igual ao identificado na planilha — sem precisar lançar nada manualmente depois
...
- **Converter em pedido (ícone de pacote, orçamentos ainda não aprovados):** abre modal com data de entrega e as parcelas de pagamento — escolha manualmente a quantidade (1x-60x, com data de vencimento cada) OU um **Plano de Pagamento por fase** (Plano A: Entrada 30% + Aprovação do Projeto no Grupo 20% + Medição 15% + Entrega do Material 15% + Montagem 10% + No Término 10%; Plano B: mesma base, mas Medição Final 15% + Entrega 10% + Montagem 15% + Término 10%) — os % incidem sobre o valor total do pedido, mas **só a Fase 1 (Entrada) é lançada**, sem data de vencimento (fica em aberto até o cliente pagar); as fases seguintes são abertas depois, uma a uma, no Financeiro (ver "Plano de Pagamento por fase" abaixo)
...
- **Contrato já em andamento:** campo **"Começar a partir da parcela nº"** — pra cadastrar um contrato que já tinha parcelas pagas fora do sistema (ex: já pagaram 1, 2 e 3 por fora, e agora quer lançar a partir da 4/8). A entrada e as parcelas anteriores à informada são criadas automaticamente já como **pagas** (com a data que elas realmente venceriam, editável depois se a data real for diferente) — não somem do histórico, ficam registradas e acessíveis pra edição igual qualquer outro lançamento. As demais entram como pendentes normalmente. Se o contrato já está **100% quitado**, coloque o número de parcelas + 1 (ex: 9 num contrato de 8x) e nada é criado
- **Plano de Pagamento por fase (sem data fixa):** ao criar um lançamento, o campo **"Plano de Pagamento"** oferece **Plano A** (Entrada 30% + Aprovação do Projeto no Grupo 20% + Medição 15% + Entrega do Material 15% + Montagem 10% + No Término 10%) e **Plano B** (mesma base, mas Medição Final 15% + Entrega 10% + Montagem 15% + Término 10%) — os % incidem sobre o **valor total do projeto**. Diferente do parcelamento normal, aqui **não existe data de vencimento**: a fase escolhida em **"Fase atual do projeto"** é lançada em aberto ("Aguardando" na coluna de vencimento), e as fases anteriores a ela entram automaticamente como **já pagas** (data de hoje, editável depois) — não somem do histórico. Quando a obra avança pra próxima etapa, clique no botão **"Avançar Fase"** (ícone de seta) na linha do lançamento — isso abre a próxima pendência (com o valor certo) sem precisar recriar nada. Se já está 100% quitado, escolha "Nenhuma — contrato já 100% quitado" e nada é lançado. **Campo Cliente obrigatório** nesse modo — sem ele a fase não aparece no Carnê de Parcelas do cliente. Ao marcar uma fase como paga, a data do pagamento é salva no lançamento e aparece automaticamente no perfil do cliente (Carnê de Parcelas → aba "Quitadas", com data e forma de pagamento)
...
- **Contas a Receber / Contas a Pagar:** por padrão mostram TODO o saldo pendente e vencido (não só o mês atual) — o filtro de data padrão vai de 3 anos atrás até 3 anos à frente, pra não esconder nem parcelamentos longos futuros nem títulos vencidos há meses. Tem botões rápidos **1 mês | 3 meses | 6 meses | 1 ano | Personalizado** pra estreitar o período — editar as datas manualmente muda automaticamente pra "Personalizado". O filtro de status **"Vencido"** mostra só o que está realmente pendente e com data no passado. Lançamentos de **Plano de Pagamento por fase** (sem data) aparecem sempre, em qualquer período escolhido, já que não têm vencimento pra filtrar
- **Cobrar atraso via WhatsApp:** linhas vencidas (pendente + data no passado) ficam destacadas em vermelho na lista e mostram um botão de WhatsApp (se o cliente tiver telefone/whatsapp cadastrado) que abre uma mensagem pronta cobrando o pagamento em atraso, com o valor e há quantos dias venceu
- **Card "Vencidos" do Dashboard:** clicar nele já abre Contas a Receber com o filtro "Vencido" aplicado (antes só trocava de aba sem filtrar). Os itens da lista "VENCIDOS" na barra lateral também são clicáveis — abrem o lançamento pra edição
- **Aba Dashboard do Financeiro:** tem um seletor de período ao lado do botão "Transferência" — **Tudo | 1 mês | 3 meses | 6 meses | 1 ano | 3 anos | Personalizado** (com campos de data quando escolhido) — que recalcula todos os cards da tela (A Receber, A Pagar, Recebido, Pago, Lucro, Receitas Previstas) para o período escolhido; os rótulos dos cards mudam pra indicar o período ativo (ex: "A Receber (Total)" quando "Tudo" está selecionado). "Saldo em Caixa" e "Vencidos" não mudam com o filtro (são sempre o valor real atual). O painel "Resumo do Mês" na lateral direita é independente e sempre mostra o mês corrente
