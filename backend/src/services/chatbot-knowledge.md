...
- **Aba Dashboard do Financeiro:** tem um seletor de período ao lado do botão "Transferência" — **1 mês | 3 meses | 6 meses | 1 ano | 3 anos | Personalizado** (com campos de data quando escolhido) — que recalcula todos os cards da tela (A Receber, A Pagar, Recebido, Pago, Lucro, Receitas Previstas) para o período escolhido; os rótulos dos cards mudam pra indicar o período ativo. "Saldo em Caixa" e "Vencidos" não mudam com o filtro (são sempre o valor real atual). O painel "Resumo do Mês" na lateral direita é independente e sempre mostra o mês corrente
